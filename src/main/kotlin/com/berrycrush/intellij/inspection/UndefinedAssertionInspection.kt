 ...
 */
private fun BerryCrushAssertElement.isBuiltInAssertion(holder: ProblemsHolder): Boolean {
    return checkAssertionCondition(holder, this.children.toList().filterIsInstance<BerryCrushPsiElement>())
}
...
private fun BerryCrushAssertElement.checkAssertionCondition(
    ...
    return elements.isNotEmpty() && if (elements[0] is BerryCrushNotElement) {
...
private fun checkAssertionOperation(element: BerryCrushAssertOperationElement, elements: List<PsiElement>): Boolean =
    when (element.operationType) {
        BerryCrushTokenTypes.CONTAINS,
        BerryCrushTokenTypes.RESPONSE_TIME -> checkSimpleCondition(element, elements)
...
private fun BerryCrushAssertElement.checkJsonPathAssertion(
    ...
    elements: List<PsiElement>): Boolean {
...
private fun checkOperatorCondition(elements: List<PsiElement>): Boolean {
    fun check(element: PsiElement, elements: List<PsiElement>, negate: Boolean = false): Boolean {
        return when (element) {
            is BerryCrushNotElement -> !negate && elements.isNotEmpty() && check(elements[0], elements.drop(1), true)
            is BerryCrushOperatorElement -> when (element.operatorType) {
                ...
                BerryCrushTokenTypes.ARRAY_SIZE -> checkValue(elements)
                BerryCrushTokenTypes.EXISTS,
                BerryCrushTokenTypes.EMPTY,
                BerryCrushTokenTypes.NOT_EMPTY -> elements.isEmpty()
                else -> false
            }
            else -> false
        }
...
private fun checkSimpleCondition(element: BerryCrushAssertOperationElement, elements: List<PsiElement>, negate: Boolean = false): Boolean {
    return elements.isNotEmpty() && if (elements[0] is BerryCrushNotElement) {
...
private fun checkHeaderCondition(elements: List<PsiElement>): Boolean {
    return if (elements.isEmpty()) {
        ...
        if (header.node.elementType != BerryCrushElementTypes.TEXT) {
            ...
            rest.isEmpty() || if (rest[0] is BerryCrushOperatorElement) {
                val op = rest[0] as BerryCrushOperatorElement
                when (op.operatorType) {
                    BerryCrushTokenTypes.COLON,
                    BerryCrushTokenTypes.EQUALS -> checkValue(rest.drop(1))
...
private fun checkValue(elements: List<PsiElement>): Boolean {
    return elements.size == 1 && elements[0] is BerryCrushPsiElement
}
